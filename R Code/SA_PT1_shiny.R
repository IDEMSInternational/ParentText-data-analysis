...
parenttext_shiny <- function(country, date_from = NULL, date_to = NULL, include_archived_data = FALSE){
  ...
  ui <- fluidPage(
    ...
    dashboardPage(
      ...
      dashboardBody(
        ...
        column(6, align = "center", checkbox_IPV(country = country)),
        #,
  ...
  server <- function(input, output) {
    ...
    output$parenting_survey_plot <- renderPlotly({
      parenting_survey <- selected_survey_data_date_from() %>% mutate(week = ifelse(week == "Baseline", 1, week),
                                                       as.numeric(week)) %>%
        filter(Group %in% c("Positive parenting", "Child maltreatment", "Stress", "Child Behaviour"))
      parenting_survey_plot <- summarySE(parenting_survey, groups = c(week, Group), var = vals, na.rm = TRUE)
    ...
    output$behaviour_plots <- renderPlotly({
      parenting_survey <- selected_survey_data_date_from() %>% filter(!is.na(vals))
      
      parenting_survey_plot <- summarySE(parenting_survey, groups = c(week), var = vals, na.rm = TRUE)
      ggplot(parenting_survey_plot, aes(x=week, y=mean, group = week), width = 2) + 
        ...
        labs(x = "Survey", y = "Frequency", title = "Survey Responses with (SE) Error Bars") +
        theme_classic() +
        scale_x_discrete(limits = c("Baseline", "2", "3", "4"))
    })
    ...
    selected_flow_data_date_from <- reactive({
      ...
      all_flows_df <- flow_data_table_function2(all_flows_df, Flow) %>% 
        return(all_flows_df)
    })
