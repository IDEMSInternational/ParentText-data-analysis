...
update_data <- function(country = "Malaysia", date_from = "2021-10-14", date_to = NULL, include_archived_data = FALSE) {
  ...
  if (country == "Malaysia"){
    ...
    stop("Invalid country given. Current accepted countries are: Malaysia")
  }
  ...
  if (is.null(contacts_unflat)){
    df <- NULL
    df_consent <- NULL
    all_flows <- NULL
    parenting_survey <- NULL
    pp_data_frame <- NULL
  } else {
    ...
    supportive_flow_names <- c("PLH - Supportive - Family", "PLH - Supportive - Help reminder", "PLH - Supportive - Share", "PLH - Supportive - Share - Enrollment",
                               "GG - PLH - Supportive - Share - Enrollment", "PLH - Supportive - Budget", "PLH - Supportive - Activities for babies", "PLH - Supportive - Activities",
    ...
    parenting_survey <- parenting_survey %>% mutate(week = fct_relevel(week, c("Baseline", "2", "3", "4", "5", "6", "7", "8", "9")))
